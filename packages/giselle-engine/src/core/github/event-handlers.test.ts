...
describe("GitHub Event Handlers", () => {
	...
	beforeEach(() => {
		...
		testDeps = {
			...
			createAndRunFlow: vi.fn().mockResolvedValue(undefined),
			parseCommand: vi
				.fn()
	...
	describe("processEvent", () => {
		it("should process event and run flow when handler returns shouldRun=true", async () => {
			...
			// Assert
			expect(testDeps.createAndRunFlow).toHaveBeenCalledWith(
		...
		it("should return false when trigger is disabled", async () => {
			...
			expect(testDeps.createAndRunFlow).not.toHaveBeenCalled();
