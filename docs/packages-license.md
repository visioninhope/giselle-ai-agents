...
As of November 5, 2024 6:54am. 947 total
...
- 1 Apache 2.0, BSD Zero Clause License, ISC, MIT, Mozilla Public License 2.0,
  New BSD, Simplified BSD, The Unlicense, zlib/libpng license
...
Babel helper for ensuring that access to a given value is performed through
simple accesses
...
Positioning library for floating elements: tooltips, popovers, dropdowns, and
more
...
Packages @jridgewell/trace-mapping and @jridgewell/gen-mapping into the familiar
source-map API
...
Audited & minimal 0-dependency JS implementation of SHA, RIPEMD, BLAKE, HMAC,
HKDF, PBKDF & Scrypt
...
Set of stateless request methods to create, check, reset, refresh, and delete
user access tokens for OAuth and GitHub Apps
...
Generated TypeScript definitions based on GitHub's OpenAPI spec for
api.github.com
...
Send parameterized requests to GitHub's APIs with sensible defaults in browsers
and Node
...
A Promise-compatible abstraction that defers resolving/rejecting promises to
another closure.
...
>>>>>>> f83567a (Update docs/packages-license.md)
...
OpenTelemetry Core provides constants and utilities shared by all OpenTelemetry
SDK packages.
...
OpenTelemetry Collector Metrics Exporter allows user to send collected metrics
...
OpenTelemetry Collector Exporter allows user to send collected traces to the
...
OpenTelemetry Collector Trace Exporter allows user to send collected traces to
...
OpenTelemetry Collector Exporter allows user to send collected traces to the
OpenTelemetry Collector using protobuf over HTTP
...
OpenTelemetry Zipkin Exporter allows the user to send collected traces to
Zipkin.
...
OpenTelemetry instrumentation for `generic-pool` resource pool for managing
expensive resources
...
OpenTelemetry instrumentation for `graphql` gql query language and runtime for
GraphQL
...
OpenTelemetry instrumentation for `node:http` and `node:https` http client and
server modules
...
OpenTelemetry instrumentation for `lru-memoizer` function memoization using
lru-cache
...
OpenTelemetry instrumentation for `mongoose` database object data modeling (ODM)
library for MongoDB
...
OpenTelemetry instrumentation for `pg` and `pg-pool` database client for
PostgreSQL
...
OpenTelemetry B3 propagator provides context propagation for systems that are
using the B3 header format
...
OpenTelemetry Jaeger propagator provides HTTP header propagation for systems
that are using Jaeger HTTP header format.
...
OpenTelemetry Node SDK provides automatic telemetry (tracing, metrics, etc) for
...
Reads / writes floats / doubles from / to buffers in both modern and ancient
browsers.
...
A Babel plugin that annotates frontend components with additional data to enrich
the experience in Sentry
...
Use the Supabase JavaScript library in popular server-side rendering (SSR)
frameworks.
...
A Tailwind CSS plugin for automatically styling plain HTML content with
...
> The license is none. Check https://github.com/route06inc/giselle/issues/12
> later
...
Speed Insights is a tool for measuring web performance and providing suggestions
for improvement.
...
A function to parse floating point hexadecimal strings as defined by the
...
A pure JavaScript W3C standard-based (XML DOM Level 2 Core) DOMParser and
...
React Flow - A highly customizable React library for building node-based editors
and interactive flow charts.
...
Matches strings against configurable strings, globs, regular expressions, and/or
functions
...
This is a marker package to indicate that a module can only be used in Client
Components.
...
A tool set for CSS: fast detailed parser (CSS → AST), walker (AST traversal),
generator (AST → CSS) and lexer (validation and matching) based on specs and
...
Define a data property on an object. Will fall back to assignment in an engine
without descriptors.
...
A library to create readable "multipart/form-data" streams. Can be used to
...
Use node's fs.realpath, but fall back to the JS implementation if the native one
fails
...
Does the environment have full property descriptor support? Handles IE 8's
...
Does this environment have the ability to get the [[Prototype]] of an object on
creation with `__proto__`?
...
Returns true if a number or string value is a finite number. Useful for regex
matches, parsing, user input, etc.
...
Returns true if an object was created by the `Object` constructor, or
Object.create(null).
...
Fast (and loose) selective `process.env` replacer using js-tokens instead of an
AST
...
<a href="http://creativecommons.org/publicdomain/zero/1.0">CC0 1.0 Universal</a>
permitted
...
Extract the Node.js module details like name and base path given an abosulte
path to a file inside the module
...
Neo-Async is a drop-in replacement for Async, it almost fully covers its
...
A better node-open. Opens stuff like websites, files, executables.
Cross-platform.
...
Blazing fast and accurate glob matcher written in JavaScript, with no
dependencies and full support for standard and extended Bash glob features,
including braces, extglobs, POSIX brackets, and regular expressions.
...
Offers getProxyForUrl to get the proxy URL for a URL, respecting the *_PROXY
...
JavaScript syntax tree transformer, nondestructive pretty-printer, and automatic
...
resolve like require.resolve() on behalf of files asynchronously and
synchronously
...
Utility method to run function either synchronously or asynchronously using the
common `this.async()` style.
...
Serialize JavaScript to a superset of JSON that includes regular expressions and
functions.
...
High performance Node.js image processing, the fastest module to resize JPEG,
PNG, WebP, GIF, AVIF and TIFF images
...
<a href="https://opensource.org/licenses/0BSD">BSD Zero Clause License</a>
permitted
...
drop-in replacement. It works with all Event-Types and accounts for basically no
...
unist utility to serialize a node, position, or point as a human readable
location
...
Backwards compatible shim for React's useSyncExternalStore. Works with any React
that supports hooks.
...
<a href="https://www.mozilla.org/media/MPL/2.0/index.815ca599c9df.txt">Mozilla
Public License 2.0</a>,
<a href="http://opensource.org/licenses/BSD-3-Clause">New BSD</a>,
<a href="http://opensource.org/licenses/bsd-license">Simplified BSD</a>,
<a href="https://unlicense.org/">The Unlicense</a>,
<a href="https://opensource.org/licenses/Zlib">zlib/libpng license</a> permitted
...
Packs ECMAScript/CommonJs/AMD modules for the browser. Allows you to split your
codebase into multiple bundles, which can be loaded on demand. Supports loaders
to preprocess files, i.e. json, jsx, es7, css, less, ... and your custom stuff.
