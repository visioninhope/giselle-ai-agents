...
const AgentWorkspace = () => {
  const files = [
    { icon: <Globe size={16} />, name: "chronobeach_first_pla", count: 4, active: true },
  ...
  const versions = ["Jul 17, 10:39 AM", "Jul 17, 10:43 AM", "Jul 17, 10:48 AM", "Jul 17, 10:54 AM"]
        ...
        <aside className="w-[300px] bg-sidebar flex flex-col p-2 border-r border-border space-y-4">
          ...
          <div className="bg-card p-3 rounded-lg space-y-3">
            ...
            <div className="flex bg-secondary rounded-md p-1 text-sm">
              <button className="flex-1 text-muted-foreground py-1 px-2 rounded">Workflow</button>
              <button className="flex-1 bg-card text-foreground py-1 px-2 rounded-md shadow-sm">Files</button>
            </div>
          </div>
          ...
          <div className="flex-grow space-y-2 overflow-y-auto">
            <div className="flex justify-between items-center px-2 py-1">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-semibold bg-secondary px-2 py-0.5 rounded-md">19</span>
              </div>
            </div>

            {files.map((file, index) => (
              <div key={index} className={`rounded-lg ${file.active ? "bg-card" : ""}`}>
                <div className={`flex items-center justify-between p-2 rounded-lg cursor-pointer hover:bg-card/80`}>
                  <div className="flex items-center gap-2">
                    <div className="text-muted-foreground">{file.icon}</div>
                    <span className="text-sm">{file.name}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-xs bg-secondary px-2 py-0.5 rounded-md">{file.count}</span>
                    {file.active ? (
                ...
                {file.active && (
                  <div className="pl-4 pr-2 pb-2">
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <button className="text-xs text-muted-foreground hover:text-foreground">Select to preview</button>
                    </div>
                    <div className="space-y-2 pl-6 border-l border-dashed border-border ml-2">
                      {versions.map((version, vIndex) => (
                        <div key={vIndex} className="flex items-center gap-2 text-sm text-muted-foreground relative">
          ...
          <div className="bg-primary/10 text-center text-sm p-2 rounded-lg">Flowith Now Open to ALL.</div>

          <div className="flex items-center justify-between p-2 border-t border-border">
            <div className="flex items-center gap-3">
              <img src="/placeholder.svg?width=40&height=40" alt="User Avatar" className="w-10 h-10 rounded-full" />
              <div>
                <p className="font-semibold">toyamarinyon</p>
                <p className="text-xs text-muted-foreground">Subscription & Settings</p>
              </div>
            </div>
            ...
            <div className="w-full max-w-2xl mx-auto flex flex-col items-center">
              ...
              <div className="bg-card border border-border rounded-2xl p-6 w-full space-y-4 shadow-lg">
                ...
                <blockquote className="border-l-2 border-primary pl-4 italic text-primary-light">
                  "This document has been updated to reflect the new protagonist, Aya, as per user feedback."
                </blockquote>
                <div className="border-t border-border"></div>
                <div className="space-y-2">
                  <h4 className="text-lg font-bold">Game Design Document: Chrono//Breach</h4>
                  <h5 className="text-md font-semibold">1. Executive Summary</h5>
                  <p className="text-muted-foreground text-sm">
                    <span className="text-primary-light">Chrono//Breach</span> is a 2D action roguelike set in a
                    collapsing digital reality. Players take on the role of Aya, a brilliant high-school student and
                    coder...
                  </p>
                </div>
