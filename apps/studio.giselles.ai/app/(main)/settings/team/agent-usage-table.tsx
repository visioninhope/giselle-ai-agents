...
export async function AgentUsageTable({
	activities,
	containerClassName,
	...
	if (settingsV2Mode) {
		return <AgentUsageTableV2 activities={activities} />;
	}
...
export function AgentUsageTableV2({
	activities,
	containerClassName,
}: AgentUsageTableProps) {
	return (
				...
				<div className="px-2">
					{activities.length > 0 ? (
						activities.map((activity) => (
							<div
								key={`${activity.agentId}-${activity.startTime}`}
								className="grid grid-cols-4 gap-1 py-2 border-b-[0.5px] border-black-400 text-white-400 font-hubotSans font-medium text-[12px] leading-[14.4px] tracking-normal"
							>
