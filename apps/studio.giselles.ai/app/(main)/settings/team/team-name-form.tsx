			...
			action={{ component: (
				<Dialog open={isEditingTeam} onOpenChange={setIsEditingTeam}>
					...
					<DialogContent
						...
						style={{ animation: 'fadeIn 0.2s ease-out', transformOrigin: 'center' }}
					>
						<style jsx global>{`
							@keyframes fadeIn {
						...
						`}</style>
						<div aria-hidden="true" className="absolute inset-0 rounded-[16px] border-[0.5px] border-transparent bg-black-900 bg-clip-padding" />
						...
						<form className="flex flex-col gap-y-4 relative z-10">
							<div className="flex flex-col gap-y-2">
								...
								/>
								{error && (
									<p className="text-[12px] leading-[20.4px] text-error-900 font-geist">{error}</p>
								)}
							</div>
							<div className="flex justify-end space-x-4">
								<Button type="button" onClick={handleCancelTeamName} disabled={isLoading} className="w-full h-[38px] bg-transparent border-black-400 text-black-400 text-[16px] leading-[19.2px] tracking-[-0.04em] hover:bg-transparent hover:text-black-400">Cancel</Button>
								<Button type="submit" disabled={isLoading || !!error} onClick={handleSaveTeamName} className="w-full h-[38px] text-[16px] leading-[19.2px] tracking-[-0.04em] ">{isLoading ? 'Saving...' : 'Save'}</Button>
							</div>
						</form>
					</DialogContent>
				</Dialog>
